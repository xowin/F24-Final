<?php
$songs = [
    [
        'cover' => 'images/Red_Like_Roses_cover.jpg',
        'title' => 'Red Like Roses (Red Trailer)',
        'album' => 'Songs Part One',
        'artist' => 'Jeff Williams, Casey Lee Williams',
        'duration' => '3:12'
    ],
    [
        'cover' => 'images/fireflies.jpeg',
        'title' => 'Fireflies',
        'album' => 'Ocean Eyes',
        'artist' => 'Owl City',
        'duration' => '3:35'
    ],
    [
        'cover' => 'images/wolf.jpg',
        'title' => 'Running with the Wolves - WolfWalkers Version',
        'album' => 'WolfWalkers (Original Motion Picture Soundtrack)',
        'artist' => 'AURORA',
        'duration' => '3:35'
    ],
    [
        'cover' => 'images/kiss.jpeg',
        'title' => 'Kiss the Sky (from The Wild Robot)',
        'album' => 'Kiss the Sky (from The Wild Robot)',
        'artist' => 'Maren Morris',
        'duration' => '3:36'
    ],
    [
        'cover' => 'images/Enemy_Imagine_Dragons.jpg',
        'title' => 'Enemy (with JID) - from the series Arcane League of Legends',
        'album' => 'Enemy (with JID) - [from the series Arcane League of Legends]',
        'artist' => 'Imagine Dragons',
        'duration' => '2:25'
    ],
    [
        'cover' => 'images/bird.jpeg',
        'title' => "Joke's On You",
        'album' => "Joke's On You",
        'artist' => 'Charlotte Lawrence',
        'duration' => '3:11'
    ],
    [
        'cover' => 'images/DreamingOutLoud.jpg',
        'title' => 'Stop And Stare',
        'album' => 'Dreaming Out Loud',
        'artist' => 'OneRepublic',
        'duration' => '3:44'
    ],
    [
        'cover' => 'images/21C.jpg',
        'title' => 'Chlorine',
        'album' => 'Trench',
        'artist' => 'Twenty One Pilots',
        'duration' => '5:24'
    ],
    [
        'cover' => 'images/Idol.jpeg',
        'title' => 'アイドル',
        'album' => 'アイドル',
        'artist' => 'YoASOBI',
        'duration' => '3:46'
    ],
    [
        'cover' => 'images/campfire.jpg',
        'title' => 'Campfire Song Song',
        'album' => "SpongeBob's Greatest Hits",
        'artist' => 'Spongebob Squarepants',
        'duration' => '0:57'
    ],
];
